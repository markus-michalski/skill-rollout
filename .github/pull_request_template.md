...
- [ ] All tests pass locally (`pytest tests/smoke -q`)
- [ ] `ruff check .` passes
...
- [ ] I have updated `reference/eval-schema.md` if tier classification logic changed

### Testing

**Automated CI checks (run automatically on all PRs):**
- pytest smoke tests (Ubuntu + Windows)
- ruff lint + format check
- JSON validation (`.claude-plugin/plugin.json`, `.mcp.json`)

**Manual testing details:**
<!-- Describe manual testing performed, if applicable.
     For run/status skills: which plugin was used as target, how many skills, any BLOCKED/READ-ONLY edge cases? -->
