...
Use GitHub's [Private Vulnerability Reporting](https://github.com/markus-michalski/skill-rollout/security/advisories/new) feature to report security issues privately. You will receive an acknowledgement within 72 hours.
...
### Workflow Script (`workflows/skill-rollout.js`)

The workflow script orchestrates multi-agent rollouts across target plugins and can spawn
many Claude agents. Changes here have a wide blast radius:
- Must include a description of what changed and why
- Must be tested against at least one real plugin rollout before merging
- Version bump required if the script changes observable behavior
...
User-specific configuration lives at `~/.skill-rollout/config.yaml` — outside this repository.
...
Python dependencies are pinned in `requirements.txt`. Dependabot scans weekly and opens PRs
for updates. When adding dependencies:
