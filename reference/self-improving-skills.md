...
#### Pflicht-Audit: globale Singleton-Invarianten, bevor `reset-testdata`/`delete-testdata` entworfen werden (Issue #61)

Ein Präfix-Guard schützt nur, WELCHE Zeile ein Tool anfasst — nicht, was ein Write auf dieser einen Zeile als Seiteneffekt auf ANDEREN, echten Zeilen auslöst. Konkret passiert bei life-hub (life-hub#16, Review-Runde 2): `persons.is_self` ist ein globaler Singleton — `update_person(is_self=True)` auf `zz-sandbox-person` läuft über `UPDATE persons SET is_self = 0 WHERE id != ?`, das damit in derselben Anweisung auch die reale Identität des Nutzers löscht (`add_person` hat denselben Effekt über `UPDATE persons SET is_self = 0 WHERE is_self = 1`). Derselbe Snapshot-Mechanismus sichert dort außerdem den aktiven Fall (`active case`) — ein zweiter, gleich gearteter Singleton in derselben Tabelle, nicht nur `is_self` allein. Der Präfix-Guard verhinderte korrekt jeden Zugriff auf fremde ZEILEN — er kann strukturell nicht verhindern, dass ein Write auf der EIGENEN (Sandbox-)Zeile über eine Anwendungs-Invariante hinweg auf fremde Zeilen ausstrahlt. Das war ein Near-Miss, kein Incident: in Runde 1 der Review sah der erste Fix (Sandbox-Zeile selbst zurücksetzen) vollständig aus und bestand alle eigenen Tests, war aber trotzdem falsch — erst Runde 2 (Breaking-Change-Analyse) fand die Lücke.

Deshalb, bevor `reset-testdata`/`delete-testdata` für ein Plugin implementiert werden: das Datenmodell explizit nach Feldern/Flags durchsuchen, bei denen ein Write auf einer Zeile andere Zeilen derselben Tabelle mit-verändert — Kandidaten: Uniqueness-Flags über Anwendungscode statt DB-Constraint (`is_active`/`is_default`/`is_primary`/`is_self`-artige Spalten, durchgesetzt per `UPDATE ... SET flag=0 WHERE flag=1`), ein globaler "aktuell/zuletzt verwendet"-Zeiger (auch eine `CHECK (id = 1)`-Singleton-Tabelle mit genau einer Zeile zählt hierzu, selbst wenn jedes Statement formal `WHERE id = 1`-gescoped ist), oder abgeleitete Aggregate/Zähler, die bei jedem Write über die ganze Tabelle neu berechnet werden. Wird so ein Feld gefunden: `reset-testdata`/`delete-testdata` MÜSSEN den Vorwert **vor** jeder Mutation sichern (Snapshot in eine Datei, nicht nur In-Memory — Test und Reset können getrennte Tool-Calls/Prozess-Invocations sein) und beim Teardown/Reset unbedingt wiederherstellen — siehe life-hub `servers/life-hub-server/tools/testdata.py`s `_snapshot_before_mutation`/`_restore_prior_self_person` als Referenzimplementierung. Wird nichts gefunden, das explizit im PR/in der Session-Doku festhalten ("Datenmodell auf globale Singleton-Invarianten geprüft, keine gefunden") statt es stillschweigend zu übergehen — ein Negativ-Befund ist genauso wertvoll wie ein Fund, aber nur, wenn er dokumentiert ist.

**Befund aus Issue #61 (2026-07-30):** Nachträglich gegen die drei anderen Ziel-Plugins geprüft — mit dem Vorbehalt, dass dies ein einmaliger Code-Scan war, keine erschöpfende Verifikation (Runde 1 von life-hubs eigener Review zeigt, dass ein Scan, der grün aussieht, trotzdem eine Invariante übersehen kann).
- **project-hub** hat tatsächlich einen zweiten Fall derselben Bugklasse: die `session`-Tabelle ist als `CHECK (id = 1)`-Singleton angelegt, `UPDATE session SET project_id = ?, ... WHERE id = 1` in `tools/session.py`. Praktisch unkritisch, weil geprüft: keiner der drei Testdata-Skills ruft den Session-Write auf, und `delete_project`s Session-Clear ist zusätzlich auf `WHERE project_id = ?` gescoped (project-hub#83) — aber die Tabelle ist ein Singleton im Sinne dieses Audits, nicht "kein Analogon".
- **storyforge** hat entgegen einer früheren Annahme in diesem Dokument sehr wohl geteilte SQLite-Tabellen (`tools/db/`: `sessions`, `author_discoveries`, `character_snapshots`, `canon_facts`, `book_rules`) — kein reines Dateisystem-Plugin. Geprüft wurden alle `UPDATE`/`DELETE`-Statements darin: durchgängig `user_id`/`author_slug`/`id`-gescoped auf genau eine Zeile, kein Statement mutiert andere Entities. Kein Singleton-Fund.
- **vidcraft** hat keine SQL-Datenbank, aber einen konsolidierten `state.json`-Index über alle Projekte (`tools/state/indexer.py`). Der Index wird bei jeder Mutation komplett aus den Quelldateien neu aufgebaut (`rebuild()`/`_cache.invalidate()`), nicht selektiv einzelne Felder umgeschrieben — hält also keine eigene, von einem Einzel-Write verletzbare Invariante. Einziger echter Singleton-Wert ist `state["session"]` (zuletzt bearbeitetes Projekt/Episode), der ausschließlich sein eigenes Feld schreibt, nie fremde Projekt-/Episode-Daten.

Fazit: life-hub und project-hub haben je einen globalen Singleton in derselben Tabelle, in der auch die Sandbox-Fixtures leben — bei life-hub griff er in einen Live-Test hinein (Near-Miss), bei project-hub bislang nicht, weil kein Testdata-Skill ihn anfasst. storyforge/vidcraft haben keinen vergleichbaren Fund. Keiner der geprüften Fälle — gefunden oder nicht — zeigt eine Invariante, die sich NICHT per Snapshot/Restore (life-hub, bereits umgesetzt) oder schlicht durch Nicht-Anfassen (project-hub) sichern ließe; das, nicht eine Trefferquote, ist das eigentliche Kriterium für die Option-B-Frage weiter unten.
...
**Re-Evaluation nach dem life-hub `is_self`-Near-Miss (Issue #61, 2026-07-30) — Option B wird NICHT höher priorisiert.** Der Near-Miss war der erste konkrete Beleg dafür, dass Option A eine strukturelle Grenze hat (ein Präfix kann eine globale Invariante nicht schützen) — das ist real und bleibt in der Doku oben stehen. Das trägt aber nicht automatisch die Schlussfolgerung "also höher priorisieren": Issue #61 argumentiert mit *Fragilität* (eine Invariante, die pro Plugin neu gefunden werden muss, wird irgendwann übersehen), nicht mit *Häufigkeit* — und Fragilität wird durch das Pflicht-Audit oben behoben, nicht durch eine Council-Entscheidung zu Option B. Die Nachprüfung gegen die drei anderen Ziel-Plugins (siehe Befund oben) fand bei project-hub einen zweiten echten Singleton in derselben Tabelle wie die Sandbox-Fixtures — bislang unkritisch, weil kein Testdata-Skill ihn schreibt, aber strukturell derselbe Bugklassen-Kandidat wie life-hubs `is_self`. Entscheidend für die Option-B-Frage ist nicht, WIE OFT ein Singleton gefunden wurde, sondern OB einer gefunden wurde, der sich nicht per Snapshot/Restore sichern lässt: bislang keiner — life-hubs `is_self`/aktiver-Fall-Paar ist vollständig durch Option A + Snapshot/Restore gelöst und ausgeliefert, project-hubs `session`-Singleton ist durch schlichtes Nicht-Anfassen unkritisch. Solange das so bleibt, ist das Pflicht-Audit oben die eigentliche Lehre aus diesem Issue — nicht ein Prioritätswechsel zu Option B, und die Nachprüfung hier ist ohnehin ein einmaliger Code-Scan, keine erschöpfende Verifikation (siehe Vorbehalt oben). Option B bleibt wie bisher: dokumentiert, zurückgestellt, Machbarkeit ungeprüft — hochstufen, sobald ein KÜNFTIGES Plugin-Audit eine globale Invariante findet, die sich nicht per Snapshot/Restore sichern lässt (z.B. weil sie nicht über einen einzelnen Read einfangbar ist, oder weil sie durch nebenläufige/racende Writes ausgelöst wird) oder die ein Testdata-Skill tatsächlich schreiben muss, ohne dass Nicht-Anfassen eine Option ist.
