...
Simulated and a Live column. Six states, not two:
...
| 🟩 READ-ONLY | Live column only — a gate-classification, not usually a resting state (issue #24). Distinct from 🟦 N/A (which means no MCP surface at all) and from 🟥 BLOCKED below: the skill DOES have a real MCP domain-tool surface, but every call it makes was verified — by checking each tool's actual documented behavior on the skill's OWN domain-tool surface (not sampled, not assumed, and never inferred from name alone) — to be incapable of creating/updating/deleting/moving/appending/writing anything. A name matching a read-verb prefix (`get_`, `list_`, `search_`, `resolve_`, `read_`) is a sanity-check starting point, not proof — e.g. a hypothetical `search_and_replace_X` name would mutate despite the `search_` prefix, so real behavior must be confirmed regardless. Any call whose real behavior can't be confirmed with confidence, or that matches none of the read prefixes, disqualifies the skill — never guessed past. One confirmed write-capable call anywhere disqualifies it entirely — no partial credit, falls back to 🟥 BLOCKED. **This tier addresses MUTATION risk only, not read-exposure risk** — a skill whose real (non-sandbox) reads would surface sensitive personal/legal/medical/business data is still 🟥 BLOCKED regardless of how clean its write-verb classification is; READ-ONLY is not a blanket "safe to run against anything real" clearance. Where it genuinely applies, a verified-read-only skill does NOT need the plugin-level `create-testdata`/`reset-testdata`/`delete-testdata` convention (issue #35) — a read has no mutation risk to sandbox against — so Prompt 3 runs directly against the real system for that skill. Because Prompt 3 typically runs in the same pass right after this classification, this symbol mostly appears transiently in a mid-run status check; once Prompt 3 completes, the row moves to ✅ (or a genuine failure state) same as any other live-tested skill, same way a pending-sandbox-convention 🟥 BLOCKED (flavor 1 below — not the permanent read-exposure flavor 2, which never resolves to ✅) eventually resolves to ✅ once the three-skill convention is implemented and verified for the plugin. |
| 🟥 BLOCKED | Live column only, and different from N/A: the skill DOES have an MCP domain-tool surface, and its Live tier cannot get a plain ✅ for one of two reasons — see the two flavors below, both of which this symbol covers. **Not** about whether the plugin's subject matter is fictional (storyforge's own shared `~/.storyforge/authors/` holds a real, non-sandbox author, `ethan-cole`, right alongside `zz-sandbox-author` — a "fictional domain" is not inherently safe).<br><br>**Two distinct flavors, and they resolve differently — annotate which one applies in the STATUS.md cell, a bare 🟥 is ambiguous about which one it is (issue #67):**<br>**1. Pending sandbox convention** (the default meaning of a bare `🟥 BLOCKED`, no annotation) — this skill has at least one write-capable call (or one not yet verified), and the *plugin* has not implemented and verified the `create-testdata`/`reset-testdata`/`delete-testdata` convention yet — per the onboarding meta-prompt's step 3a (issue #35). What actually makes a plugin unblocked is a concrete, checkable artifact, not a design conversation: all three of `create-testdata`/`reset-testdata`/`delete-testdata` exist for this plugin, each documents an unconditional `zz-sandbox-`-prefix refuse-and-stop as its own first step (verified by reading the actual SKILL.md instruction text, not trusting the skill's name or its own prose claims), AND a live call to `delete-testdata` with a synthetic, provably-nonexistent, non-`zz-sandbox-`-prefixed slug produced one of the two zero-risk outcomes (immediate prefix-refusal, or a harmless "not found" on a target that was never real) — full mechanic in `reference/prompt-self-improving-skill-playbook.md` Phase 1 step 3a and `reference/self-improving-skills.md`'s "create-testdata / reset-testdata / delete-testdata Convention" section. Default is BLOCKED for every plugin until all three of those checks have actually passed, regardless of how low-stakes its domain sounds — UNLESS the skill's own surface first clears the 🟩 READ-ONLY check above (mutation risk AND read-exposure risk both cleared), which bypasses this requirement per-skill regardless of the plugin-level default. Non-terminal: fail open toward re-checking it, since a later batch may unblock it once the convention lands. The Select phase's own done-check (`workflows/skill-rollout.js`) must keep re-selecting a skill in this state until it does.<br>**2. Permanent read-exposure** — source-verified (never guessed) that this skill's real, non-sandbox reads would surface sensitive personal/legal/medical/business data by design, independent of the plugin-wide convention's own state and independent of whether the skill also has write-capable calls elsewhere covered by flavor 1: a read has nothing to snapshot/restore against, so no sandbox design ever resolves it, no matter how thoroughly the three-skill convention above gets implemented. Write the annotation `🟥 BLOCKED (read-exposure, permanent)` into the cell (extra trailing detail after it, e.g. a case-count split like "4/6 executed, 2 permanently blocked", is fine — matching is by presence of the annotation, not cell equality). Terminal for scheduling purposes only: the Select phase's done-check recognizes this annotation and stops re-selecting the skill for it, the same way it treats ✅ or a verified 🟦 N/A — the cell itself still never becomes ✅, only the re-selection stops. Confirmed concrete case: life-hub's `new-case`/`resume`/`dashboard`/`search`, whose unconditional `tool_list_persons`/`tool_list_shared_contacts`/cross-case reads stayed blocked even after life-hub's own three-skill convention was implemented and verified plugin-wide — proof the two flavors are genuinely independent, not just two names for the same wait, and that a skill can carry flavor 2 regardless of how flavor 1 resolves for its other calls.<br><br>In all cases: never a bare ⬜ (indistinguishable from "not attempted yet"), never auto-resolved to ✅ by either flavor (flavor 2 stops being re-selected but never becomes ✅), never guessed past, and never unblocked by an undefined "conversation" step. |
...
| ↺ no-restore-accepted-drift | in the Notes column, not its own column — a specific live case called a `global-singleton` OR `shared-mutable-per-entity` MCP tool (per `{skillEvalsDir}/{plugin}/mcp-surface-register.md`'s Tool Scope table, see `self-improving-skills.md`'s "MCP Surface Register" section) whose value nothing downstream reads, so no snapshot/restore was attempted and none is needed. **This is an expected, resolved outcome, not an open question** — unlike 🟨 NEEDS-HUMAN-REVIEW, it does NOT ask a human to look at anything; it records a deliberate policy decision so a later reader doesn't mistake the absence of a restore for a bug. Distinct from a fixture-completeness block (a case that genuinely could not run because a required fixture state was missing) — that case gets a NAMED gap in Notes and does NOT get this symbol, since it wasn't run at all, let alone downgraded. |
...
own for something touching real personal/legal/business data. Without the distinct READ-ONLY tier,
a whole class of skills — anything that only reads from a live system as a lookup/context step,
with zero mutation risk — gets needlessly stuck behind sandbox-design work that doesn't actually
apply to it (confirmed concrete case: mm-skills' `socialcraft`, whose only MCP calls are
`wikijs_search_pages`/`wikijs_get_page`/`tool_get_project()` — all read-only lookups — was blocked
alongside `shopware-produkt-wizard`, which genuinely creates/updates real shop products, before
this tier existed).
